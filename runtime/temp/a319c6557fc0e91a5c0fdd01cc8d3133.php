<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:56:"D:\www\admin/app/admin\view\organization\joincircle.html";i:1520010151;}*/ ?>
<!DOCTYPE html>
<html>
        ...
        <div class="layui-field-box table-responsive">
            <table class=" layui-table table-hover">
                ...
                <tr>
                    ...
                    <td>
                        <?php if($v['oap_status'] == '未审核'): if(ORG_ID == 0): ?>
                        <a href="javascript:;" onclick="return sta('<?php echo $v['oap_id']; ?>')" class="layui-btn layui-btn-mini">通过</a>
                        <a href="javascript:;" onclick="return nsta('<?php echo $v['oap_id']; ?>')" class="layui-btn layui-btn-mini layui-btn-danger">不通过</a>
                        <?php endif; if(ORG_ID != 0): ?>
...
<script>
    ...
    function sta(id) {
        layer.confirm('确定通过？操作后不能更改！', {icon: 3}, function (index) {
            window.location.href = "<?php echo url('joincircleStatus'); ?>?oap_id="+id+"&sta=3";
        });
    }
    function nsta(id) {
        layer.confirm('确定不通过？操作后不能更改！', {icon: 3}, function (index) {
            window.location.href = "<?php echo url('joincircleStatus'); ?>?oap_id="+id+"&sta=2";
        });
    }
</script>
</body>
